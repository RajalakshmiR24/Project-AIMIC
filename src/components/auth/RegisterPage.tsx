...
import { Shield, Eye, EyeOff, Mail, Lock, User, Building2, AlertCircle, CheckCircle2 } from "lucide-react";
...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const role = decodedRole ?? createdUser.role;
  ...
  const roleOptions = [
    { value: "employee", label: "Employee", icon: <User className="w-4 h-4" />, description: "Submit and track claims" },
    { value: "doctor", label: "Doctor", icon: <User className="w-4 h-4" />, description: "Submit medical reports" },
    { value: "insurance", label: "Insurance", icon: <Building2 className="w-4 h-4" />, description: "Review and approve claims" },
  ];
